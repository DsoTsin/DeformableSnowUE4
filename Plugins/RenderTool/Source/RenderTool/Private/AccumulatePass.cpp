    ...
    FVector4 Offset, FVector4 Sizes)
{
    ...
    FRHIRenderTargetView ColorView = FRHIRenderTargetView(OutputTexture,
    ...
    AccumuPS->SetParameters(RHICmdList, Offset, Sizes, 
        InputTexture->TextureRHI, SamplerStateLinear, 
        InputTextureLast->TextureRHI, SamplerStateLinear);
    DrawPrimitiveUP(RHICmdList, PT_TriangleStrip, 2, Quad, sizeof(Quad[0]));
    RHICmdList.CopyToResolveTarget(OutputTexture, InputTexture->TextureRHI, FResolveParams());
}